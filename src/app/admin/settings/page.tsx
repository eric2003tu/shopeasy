    ...
    <div className="space-y-4">
      ...
      <div className="max-w-2xl bg-card p-6 rounded shadow-sm">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            <label className="text-sm block mb-1">Notifications</label>
        ...
        <div className="flex items-center justify-end gap-3 mt-4">
          <Button variant="outline" onClick={() => { localStorage.removeItem(STORAGE_KEY); setSettings({ theme: 'system', accent: '#7c3aed', notifications: true, currency: 'USD' }); }}>Reset</Button>
          <Button onClick={onSave}>Save</Button>
        </div>
      </div>

      <ConfirmDialog open={openConfirm} message="Settings saved" onConfirm={() => setOpenConfirm(false)} onCancel={() => setOpenConfirm(false)} />
    </div>
  );
}
